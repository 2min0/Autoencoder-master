...
def plot_histograms(corr, conf, bins=50, norm_hist=True):
    ...
    plt.ylabel('Density')
    plt.legend()
    plt.savefig('results/[train]weightloss_minibatch_revised_b_' + str(args.batch_size) + '_n_' + str(args.noise_level)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        # Save checkpoint
        if is_best:
            torch.save(model.state_dict(), 'models/BEST_checkpoint_weightloss_minibatch_revised' + '_b_' + str(args.batch_size)
