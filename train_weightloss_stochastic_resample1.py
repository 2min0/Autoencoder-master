...
parser.add_argument('--pkl', '-p', type=str, required=True, help='pickle file name (in MODELS folder)')
...
def main():
    ...
    weight_list = [1e-6 if i < 1e-6 else i for i in sample_weights]
    sampler = data.WeightedRandomSampler(weight_list, len(weight_list), replacement=True)
    ...
    train_loader = DataLoader(dataset=train_data, batch_size=args.batch_size, shuffle=True, sampler=sampler, pin_memory=True)
